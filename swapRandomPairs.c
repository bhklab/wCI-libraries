...
void swap_random(double *vector, uint64_t index, uint64_t index2){
...
SEXP do_random_swaps_c(SEXP pvector,
                  SEXP pvlen,
                  SEXP pnswap,
                  SEXP pseed){
  ...
  for(double i = 0; i < nswap; i++){
    ...
    index2  = generate_random_index(state, vector_length - 1);
  	swap_random(vector, index, index2);
  }
...
static const R_CallMethodDef callMethods[]  = {
  {"do_swaps_c", (DL_FUNC) &do_random_swaps_c, 3},
  {NULL, NULL, 0}
};
